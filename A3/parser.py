...
class PartialParse(object):
    ...
    def complete(self):
        ...
        '''
        # the parse is complete when the stack is empty and next is the size of the sentence
        ### BEGIN YOUR CODE
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id == self.left_arc_id:
            ...
            raise ValueError('Not a valid transition_id')
        ### END YOUR CODE
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        deps = []
        if len(self.arcs) == 0:
            raise ValueError("No arcs yet")
        for tup in self.arcs:
            if tup[0] == sentence_idx:
                deps.append(tup[1])
        if n is None or len(deps) < n:
            return deps
        else:
            return deps[:n]
        ### END YOUR CODE
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        l_deps_list = [dep for dep in l_deps]
        ...
        if not head:
            head = 0 
        if dir_left_dep and len(self.stack) > 2 and current in self.stack:
            for d in list(graph.nodes[current]['deps']):
                if dir_left_dep in graph.nodes[current]['deps'][d]:
                    transition_id = self.left_arc_id
                    deprel = d
                    break
        elif not dir_right_dep and head in self.stack and len(self.stack) > 1 and current in self.stack:
            for d in list(graph.nodes[head]['deps']):
                if current in graph.nodes[head]['deps'][d]:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i, transition in enumerate(minibatch_transitions):
            try:
                minibatch[i].parse_step(transition[0], transition[1])
                if minibatch[i].complete:
                    unfinished_parses.remove(minibatch[i])
            except ValueError:
                unfinished_parses.remove(minibatch[i])
    return [pp.arcs for pp in partial_parses]
    ### END YOUR CODE
