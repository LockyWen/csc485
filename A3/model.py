...
class ParserModel(Model):
    ...
    def add_embeddings(self):
        ...
        ** Embedding matrices should be variables, not constants! **
        ...
        word_embed = tf.get_variable("word_embeddings", initializer=tf.constant_initializer(self.word_embeddings), shape=[self.config.n_word_ids, self.config.embed_size])
        tag_embed = tf.get_variable("tag_embeddings", initializer=xavier, shape=[self.config.n_tag_ids, self.config.embed_size])
        deprel_embed = tf.get_variable("deprel_embeddings", initializer=xavier, shape=[self.config.n_deprel_ids, self.config.embed_size])

        word_lookup = tf.nn.embedding_lookup(word_embed, self.word_id_placeholder)
        tag_lookup = tf.nn.embedding_lookup(tag_embed, self.tag_id_placeholder)
        deprel_lookup = tf.nn.embedding_lookup(deprel_embed, self.deprel_id_placeholder)
